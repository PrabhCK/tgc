...
@Component({
  ...
  standalone: true,
  imports: [CardModule, FormsModule, RadioButtonModule, ButtonModule, TableModule, CheckboxModule, CommonModule, SelectButtonModule, ToastModule, ConfirmDialogModule],
...
export class PositionsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    this.socket.on('data', (dt: any) => {
      ...
      if (this.newposition.length > 0) {
        ...
        this.totalm2m = this.newposition.reduce((sum, sp) => sum + (sp.pnl || 0), 0);
      }
    });
  ...
  getpositions() {
    if (this.userdata?.packagename == 'PRO TRADER') { //if usr
      ...
      this.pService.getpositions().subscribe({
        next: (res: any) => {
  ...
  private filterPositions() {
    ...
    // Always assign new array reference for PrimeNG change detection
