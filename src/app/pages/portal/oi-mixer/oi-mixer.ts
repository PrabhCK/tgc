...
@Component({
  ...
  templateUrl: './oi-mixer.html',
  styleUrl: './oi-mixer.scss',
  providers:[MessageService]
})
export class OiMixer implements OnInit {
  ...
  // baseurl = environment.baseurl + '/options/getOiMixer';
  ...
  getOiMixer() {
    if (this.stockname && this.timeframe && this.strikeprice && this.expiry_selected_date && this.selected_date) {
      this.pservice.getOiMixer(this.stockname, this.timeframe, this.strikeprice, this.expiry_selected_date, moment(this.selected_date).format('YYYY-MM-DD'))
        .subscribe(
          (res: any) => {
            this.tdata = res;
